# 完整升級版 app.py（含圖片上傳、發票記帳、個人團體記帳、自動補差額、補發票、對獎）
...
def handle_message(event):
    ...
    try:
        if msg == "指令說明":
            ...
            df = get_group_records_by_group(group)
            to_delete = df[(df["Date"] == date) & (df["Meal"] == meal)]
            if to_delete.empty:
                reply = f"⚠️ 無 {group} 在 {date} {meal} 的紀錄"
            else:
                ok = all(delete_group_record_by_index(group, i) for i in to_delete.index)
                reply = f"✅ 已刪除 {group} {date} {meal}" if ok else "⚠️ 部分失敗"
